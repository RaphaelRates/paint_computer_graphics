 ...
 * @details Define os estados de animação do gatinho
 */
typedef enum {
...
// Variáveis para animação do gatinho
...
// Protótipos de funções para o gatinho
void drawCat();
...
// Funções para a animação do gatinho
void drawCat() {
    glPushMatrix();
    ...
    if (catState == CAT_MIRROR) {
        if (mirrorDirection == 0) {
            glScalef(-1.0f, 1.0f, 1.0f);  // Espelhamento horizontal
        } else {
            glScalef(1.0f, -1.0f, 1.0f);  // Espelhamento vertical
        }
    }
    ...
    // Desenhar o corpo do gatinho (círculo)
...
void updateCatAnimation() {
    ...
    // Atualizar variáveis de animação baseadas no estado
    switch (catState) {
        case CAT_IDLE:
        ...
        case CAT_TRANSLATE:
            ...
            catY = 550.0f + sinf(catAnimationTime * 8.0f) * 10.0f;
    ...
    catBlinkCounter++;
    if (catBlinkCounter > 100) {  // Aproximadamente a cada 3 segundos
        int shouldBlink = rand() % 10;  // 10% de chance de piscar
        if (shouldBlink == 0) {
            catIsBlinking = 1;
            catBlinkCounter = 0;
        }
    } else if (catBlinkCounter > 5 && catIsBlinking) {  // Duração do piscar
        catIsBlinking = 0;
    }
...
void setCatState(CatAnimationState state) {
    catState = state;
    catAnimationTime = 0.0f;  // Reinicia o tempo da animação

    // Efeitos de inicialização para diferentes estados
    switch (state) {
        case CAT_IDLE:
            catScaleFactor = 1.0f;
            catAngle = 0.0f;
...
void init() {
    ...
    // Inicializar variáveis para a animação do gatinho
    lastFrameTime = clock();
    srand(time(NULL));  // Inicializa o gerador de números aleatórios para piscar
}
